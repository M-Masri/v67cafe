...
function OrderNowModal({
  ...
  const filteredCarBrands = useMemo(() => {
    ...
    return carBrands.filter((brand) => brand.name.toLowerCase().includes(normalizedQuery))
  }, [brandSearch])
  ...
  useEffect(() => {
    ...
    const closeBrandMenuOnOutsideClick = (event) => {
      if (brandSelectRef.current && !brandSelectRef.current.contains(event.target)) {
        setBrandMenuOpen(false)
      }
    }

    window.addEventListener('mousedown', closeBrandMenuOnOutsideClick)

    return () => {
      window.removeEventListener('mousedown', closeBrandMenuOnOutsideClick)
    }
  }, [isOpen])
  ...
  return (
    <div className="order-modal-backdrop" role="presentation" onClick={onClose}>
      ...
      >
                ...
                <div className="order-car-row">
                  <label>
                    ...
                    <div className="order-brand-select" ref={brandSelectRef}>
                      <button
                        type="button"
                        className={`order-brand-select-trigger ${brandMenuOpen ? 'open' : ''}`}
                        onClick={() => setBrandMenuOpen((current) => !current)}
                        aria-haspopup="listbox"
                        aria-expanded={brandMenuOpen}
                      >
                        {selectedBrand ? (
                          <span className="order-brand-value">
                            <BrandLogo logo={selectedBrand.logo} name={selectedBrand.name} />
                      ...
                      {brandMenuOpen ? (
                        <div className="order-brand-menu">
                          <input
                            type="text"
                            value={brandSearch}
                          ...
                          <div className="order-brand-options" role="listbox">
                            {filteredCarBrands.map((brand) => (
                              <button
                                key={brand.name}
                                type="button"
                                className={`order-brand-option ${checkoutForm.car_type === brand.name ? 'active' : ''}`}
                                onClick={() => {
                                  setCheckoutForm({ ...checkoutForm, car_type: brand.name })
                                  setBrandMenuOpen(false)
                                }}
                              >
        ...
        <footer className={`order-modal-footer ${step === 1 ? 'single-action' : ''}`}>
          {step > 1 ? (
            <button
              type="button"
